...
plt.figure()
plt.imshow(row_data, vmin=0, vmax=vmax, cmap='coolwarm')
c = plt.colorbar()
c.set_label('(S/1.7e10)')
plt.tight_layout()
plt.savefig('/disk/lif2/spike/det_figs/' + detector + '/' + pathsplit[-3] + '_row_map.pdf')
plt.close()

plt.figure()
plt.imshow(col_data, vmin=0, vmax=vmax, cmap='coolwarm')
c = plt.colorbar()
c.set_label('(S/1.7e10)')
plt.tight_layout()
plt.savefig('/disk/lif2/spike/det_figs/' + detector + '/' + pathsplit[-3] + '_col_map.pdf')
plt.close()