...
for T in Tlist:
	...
	for HV in CPlist:
		...
		bot, top = plt.ylim()
		left, right = plt.xlim()
		plt.text(right*0.5, top*0.8, 'Mean = ' + str(round(np.mean(masked), 2)) +r'\,pA', fontsize = 16)
		plt.text(right*0.5, top*0.6, '1-Sigma = ' + str(round(np.std(masked), 2)) +r'\,pA', fontsize = 16)
		plt.tight_layout()
