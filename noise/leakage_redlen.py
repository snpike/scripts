...
for T in Tlist:
	...
	for HV in CPlist:
		...
		plt.text(plt.get_xlim()*0.5, plt.get_ylim()*0.8, 'Mean = ' + str(round(np.mean(masked), 2)) +r'\,pA', fontsize = 16)
		plt.text(plt.get_xlim()*0.5, plt.get_ylim()*0.6, '1-Sigma = ' + str(round(np.std(masked), 2)) +r'\,pA', fontsize = 16)
		plt.tight_layout()
