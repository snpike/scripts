...
nuproducts indir=./"$OBSID"_pipe_out/ instrument=FPMA steminputs=nu"$OBSID" outdir=./timing_products srcregionfile=./"$OBSID"_pipe_out/"$srcA" bkgregionfile=./"$OBSID"_pipe_out/"$bkgA".reg binsize=0.0005 barycorr=yes write_baryevtfile=yes orbitfile=./"$OBSID"_pipe_out/nu"$OBSID"A.attorb clockfile="$clock" srcra_barycorr="$srcA_ra" srcdec_barycorr="$srcA_dec" clobber=yes >& nuproducts_A_timing.log &
nuproducts indir=./"$OBSID"_pipe_out/ instrument=FPMB steminputs=nu"$OBSID" outdir=./timing_products srcregionfile=./"$OBSID"_pipe_out/"$srcB" bkgregionfile=./"$OBSID"_pipe_out/"$bkgB".reg binsize=0.0005 barycorr=yes write_baryevtfile=yes orbitfile=./"$OBSID"_pipe_out/nu"$OBSID"B.attorb clockfile="$clock" srcra_barycorr="$srcA_ra" srcdec_barycorr="$srcA_dec" clobber=yes >& nuproducts_B_timing.log &
