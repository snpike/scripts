...
for j in range(n):
	...
	plt.savefig('/Volumes/LaCie/CdTe/gammaFlood/images/movieFrames/gammaSimAm_step' + str(j+1) + '.png')
	plt.close()
	if ((j+1) % 10 == 0):
		print(popt[1])
		plt.figure()
		...
		plt.ylabel('Counts')
		plt.plot(range(3500), simSpec)
		plt.savefig('/Volumes/LaCie/CdTe/gammaFlood/images/gammaSimAm_step' + str(j+1) + '.eps')
		plt.close()
plt.figure()
plt.xlabel('Total Counts')
plt.ylabel('Centroid Error')
plt.plot(counts[5:], fitError[5:])
plt.savefig('/Volumes/LaCie/CdTe/gammaFlood/images/gammaSimAm_error.eps')
plt.close()
