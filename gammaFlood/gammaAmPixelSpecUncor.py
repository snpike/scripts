...
while data.field('TEMP')[i] < -50:
    i += 1
START = i
print(data.field('TIME')[-1] - data.field('TIME')[START])
...
for x in range(32):
	for y in range(32):
		...
		plt.xlabel('Channel')
		plt.ylabel('Counts')
		plt.savefig('/Volumes/LaCie/CdTe/gammaFlood/images/pixelSpectraAmUncor/20170315_H100_gamma_Am241_-10C_x' + str(x) + '_y' + str(y) +'_spectrum.0V.eps')
		plt.close()
...
while i < len(data.field('PH')):
    temp = data.field('PH_COM')[i].reshape(3,3)
    if data.field('STIM')[i] or np.sum(temp) <= 0 or (data.field('GRADE')[i]!=0):
...
images = [[imgio.imread('/Volumes/LaCie/CdTe/gammaFlood/images/pixelSpectraAmUncor/20170315_H100_gamma_Am241_-10C_x' + str(j) + '_y' + str(i) +'_spectrum.0V.eps') for j in range(32)] for i in range(32)]
