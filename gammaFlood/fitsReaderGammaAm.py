...
gainOffset =  readsav('/Volumes/LaCie/CdTe/CdTeData/h100_-10C_500V_gain.sav')['calstruct']
PHGainOffset = readsav('/Volumes/LaCie/CdTe/CdTeData/h100_-10C_500V_fit_2.sav')['calstruct3'].reshape(32,32)
...
file = fits.open('/Volumes/LaCie/CdTe/gammaFlood/20170315_H100_gamma_Am241_-10C.0V.fits')
data = file[1].data
file.close()
START = 0
i = 0
events = {'channel': [], 'rawx': [], 'rawy': [], 'grade': [], 'stim': []}
...
START = i
'''
print(data.field('STIM')[START:START + 10])
pixelMap = np.histogram2d(data.field('RAWX'), data.field('RAWY'), bins = [32,32])[0]
while i < len(data.field('PH')):
	if data.field('STIM')[i]:
		pixelMap[10,10] -= 1
	i += 1
for j in range(32):
	for k in range(32):
		if pixelMap[j,k] > 32500:
			pixelMap[j,k] = 0
heatmap = plt.imshow(pixelMap.transpose(), origin = 'upper')
plt.colorbar(heatmap, label = 'Counts')
plt.savefig('/Volumes/LaCie/CdTe/gammaFlood/images/20170315_H100_gamma_Am241_-10C_heatmap.0V.eps')
plt.close()
...
plt.savefig('/Volumes/LaCie/CdTe/gammaFlood/images/20170315_H100_gamma_Am241_-10C_pixelHistogram.0V.eps')
plt.close()'''

while i < len(data.field('PH')):
	stim = data.field('STIM')[i]
	...
	if np.sum(temp) > 0:
		...
		if (not np.isnan(channel)):
			...
			events['stim'].append(stim)
	i += 1


pickle.dump(events, open('/Volumes/LaCie/CdTe/gammaFlood/20170315_H100_gamma_Am241_-10C_dump.0V.pkl', 'wb'))
...
'''
events = pickle.load(open('/Volumes/LaCie/CdTe/gammaFlood/20170315_H100_gamma_Am241_-10C_dump.0V.pkl', 'rb'))
...
plt.savefig('/Volumes/LaCie/CdTe/gammaFlood/images/20170315_H100_gamma_Am241_-10C_spectrum.0V.eps')
plt.close()
...
plt.savefig('/Volumes/LaCie/CdTe/gammaFlood/images/20170315_H100_gamma_Am241_-10C_spectrum_keV.0V.eps')
plt.close()
...
plt.xlabel('Energy (keV)')
plt.ylabel('Normalized Counts')
plt.savefig('/Volumes/LaCies/CdTe/gammaFlood/images/20170315_H100_gamma_Am241_-10C_fit_HIGH_keV.0V.eps')
plt.close()
...
plt.colorbar(heatmap, label = 'Counts')
plt.savefig('/Volumes/LaCie/CdTe/gammaFlood/images/20170315_H100_gamma_Am241_-10C_heatmap_40-70keV.0V.eps')
plt.close()

plt.hist(pixelMap.flatten(), bins = 30, histtype = 'step')
plt.xlabel('Counts')
plt.ylabel('Pixels')
plt.savefig('/Volumes/LaCie/CdTe/gammaFlood/images/20170315_H100_gamma_Am241_-10C_pixelHistogram_40-70keV.0V.eps')
plt.close()'''
